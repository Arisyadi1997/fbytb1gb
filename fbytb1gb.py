...
# Atur batas upload max 1000MB
st.set_option('server.maxUploadSize', 1000)


def run_ffmpeg(video_path, stream_key, platform, is_shorts, log_callback):
    # RTMP URL untuk YouTube atau Facebook
    if platform == "YouTube":
...
def main():
    st.set_page_config(
        page_title="Live Streaming (YouTube / Facebook)",
        page_icon="🎥",
        layout="wide"
    )
    st.title("🎥 Live Streaming (YouTube & Facebook)")
    ...
    # Pilih platform
    platform = st.radio("Pilih Platform Streaming:", ["YouTube", "Facebook"])
    ...
    uploaded_file = st.file_uploader(
        "Upload video baru (mp4/flv - codec H264/AAC, max 1000MB)",
        type=['mp4', 'flv']
    )
    ...
    # Input stream key
    stream_key = st.text_input(f"Stream Key {platform}", type="password")
    ...
    if st.button("Mulai Streaming"):
        if not video_path or not stream_key:
            ...
            st.session_state['ffmpeg_thread'] = threading.Thread(
                target=run_ffmpeg, args=(video_path, stream_key, platform, is_shorts, log_callback), daemon=True)
            st.session_state['ffmpeg_thread'].start()
            st.success(f"Streaming dimulai ke {platform}!")
